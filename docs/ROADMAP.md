...
- [ ] 관리자 대시보드 테이블의 액션 컬럼 내 모든 요소가 수직 중앙 정렬된다
- [ ] 새로고침 버튼 클릭으로 전체 페이지 리로드 없이 견적서 목록을 갱신할 수 있다
- [ ] 견적서 목록이 10개 단위로 페이지네이션되어 표시된다
...
**완료 기준**:
- [ ] 같은 행(row)에서 상태 드롭다운과 이메일 버튼의 수직 중앙이 일치한다
- [ ] 변경 후 `StatusChangeButton`의 클릭 영역과 `SendEmailButton`의 클릭 영역이 모두 정상 동작한다
...
- [ ] **B-1. 액션 컬럼 컨테이너 정렬 수정** | 담당: 프론트엔드 | 예상: 0.5d | 우선순위: 높음 | **미완료** — `StatusChangeButton` 트리거에 `h-auto p-0` 유지 중, `h-8` 명시 필요
...
**완료 기준**:
- [ ] 대시보드 상단 헤더 영역에 새로고침 버튼이 표시된다
- [ ] 버튼 클릭 시 `router.refresh()`가 호출되어 Notion에서 최신 데이터를 다시 불러온다
- [ ] 로딩 중 버튼에 스피너가 표시된다
- [ ] 새로고침 완료 후 스피너가 사라진다
...
- [ ] **C-1. RefreshButton 클라이언트 컴포넌트 생성** | 담당: 프론트엔드 | 예상: 0.5d | 우선순위: 높음 | **미완료** — `components/admin/RefreshButton.tsx` 파일 없음
...
- [ ] **C-2. 관리자 대시보드 헤더에 RefreshButton 배치** | 담당: 프론트엔드 | 예상: 0.5d | 우선순위: 높음 | **미완료** — `admin/page.tsx` 헤더에 RefreshButton 미배치
...
**완료 기준**:
- [ ] 필터링된 결과가 10개를 초과할 때 페이지네이션 UI가 표시된다
- [ ] 이전/다음 버튼과 페이지 번호 버튼이 표시된다
- [ ] 현재 페이지 번호가 강조 표시된다
- [ ] 검색어 또는 상태 필터 변경 시 자동으로 1페이지로 초기화된다
- [ ] 총 페이지 수와 현재 표시 범위(예: "11-20 / 35건")가 표시된다
